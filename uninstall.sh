...
directories=(
    ...
    "gtk-3.0" "GTK3 theme configuration" "off"
    "gtk-4.0" "GTK4 theme configuration" "off"
)
...
# Check for AGS if installed
if [ -f "/usr/local/bin/ags" ]; then
    sudo rm "/usr/local/bin/ags"
    echo "ags removed."
fi
...
# Ask about removing Zsh and Oh-My-Zsh
if rpm -q zsh &> /dev/null; then
    if whiptail --title "Remove Zsh?" --yesno \
        "Do you also want to remove Zsh shell and Oh-My-Zsh?\n\nThis will remove:\n- zsh package\n- ~/.zshrc\n- ~/.p10k.zsh\n- ~/.oh-my-zsh/\n\nWARNING: Only do this if you don't use Zsh for other purposes!" 16 70; then
        echo "$INFO Removing Zsh and related files..." | tee -a "$LOG"
        sudo dnf remove -y zsh
        [ -f ~/.zshrc ] && rm -f ~/.zshrc && echo "$OK Removed ~/.zshrc"
        [ -f ~/.p10k.zsh ] && rm -f ~/.p10k.zsh && echo "$OK Removed ~/.p10k.zsh"
        [ -d ~/.oh-my-zsh ] && rm -rf ~/.oh-my-zsh && echo "$OK Removed ~/.oh-my-zsh/"
        echo "$OK Zsh removed."
    else
        echo "$INFO Keeping Zsh."
    fi
fi

printf "\n%.0s" {1..1}

# Ask about removing wallpapers
if [ -d ~/Pictures/wallpapers ]; then
    if whiptail --title "Remove Wallpapers?" --yesno \
        "Do you want to remove the wallpaper collection?\n\n~/Pictures/wallpapers/\n\nWARNING: This may contain your personal wallpapers!" 12 70; then
        echo "$INFO Removing wallpapers..." | tee -a "$LOG"
        rm -rf ~/Pictures/wallpapers
        echo "$OK Wallpapers removed."
    else
        echo "$INFO Keeping wallpapers."
    fi
fi
...
echo "$INFO Cleaning up leftover dependencies..." | tee -a "$LOG"
