...
# Function for displaying notifications and update Waybar file
notification() {
  notify-send -u normal -i "$iDIR/music.png" "Now Playing:" "$@"
  # write current song for Waybar compatibility
  mkdir -p /tmp/rmpc 2>/dev/null || true
  echo "$@" > /tmp/rmpc/current_song 2>/dev/null || true
}
