...
try:
    ...
    # Fallback if the HTML structure changed
    status_code = "default"
...
# print waybar module data - Multiple detailed format options
# Option 1: Full detailed
# Try multiple ways to get city name
city_full = html_data("h1").text()
if city_full:
    # Extract just the city name (first part before comma)
    city_name = city_full.split(',')[0]
else:
    # Fallback: extract from title
    title_text = html_data("title").text()
    if "Weather forecast and conditions for" in title_text:
        city_name = title_text.split("Weather forecast and conditions for ")[
            1].split(",")[0]
    else:
        city_name = "Unknown"

detailed_text = f"{city_name}: {icon} {temp} ({temp_min}/{temp_max}-{temp_feel}) 🌬️AQI{air_quality_index} 💧{humidity} 💨{wind_speed}"

# Option 2: Compact detailed
compact_detailed = f"{icon} {temp} 💧{humidity} 💨{wind_speed}"

# Option 3: Temperature focused
temp_focused = f"{icon} {temp} (feels {temp_feel}) {temp_min}-{temp_max}"

# Option 4: Weather + air quality
weather_aqi = f"{icon} {temp} 🌬️AQI{air_quality_index} 💧{humidity}"

# Choose which format to use (change this line to switch formats)
chosen_format = detailed_text
