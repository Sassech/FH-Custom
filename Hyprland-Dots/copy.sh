...
waybar_style="$HOME/.config/waybar/style/[Extra] Mauve.css"
waybar_config="$HOME/.config/waybar/configs/[TOP & BOT] SummitSplit"
waybar_config_laptop="$HOME/.config/waybar/configs/[TOP & BOT] SummitSplit" 
...
DIR="btop cava hypr Kvantum qt5ct qt6ct wallust wlogout"
...
# AGS config has been removed from this project - only Waybar is supported
...
printf "\n%.0s" {1..1}
echo "${MAGENTA}Skipping additional wallpapers download for Fedora Minimal installation${RESET}"
echo "${NOTE} You can manually download them later from: https://github.com/JaKooLit/Wallpaper-Bank.git" 2>&1 | tee -a "$LOG"

# CLeaning up of ~/.config/ backups (skipped for minimal installation)
echo "${NOTE} Backup cleanup skipped for Fedora Minimal installation" 2>&1 | tee -a "$LOG"
